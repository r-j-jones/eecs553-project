...
% for ii=1:d-r
    ...
    oi = randsample(d,r+1);
    if nnz(sum(ismember(omega_is,oi),1)==(r+1))>0
...
parfor (j=1:N, 10)
    if verbose>0 && mod(j,1000)==0, fprintf('\tj=%d\n',j); end
    warning('off','all');
    %tic
...
%     lb=median(Mj)-std(Mj);
%     ub=median(Mj)+std(Mj);
%     validInds=find(Mj<ub & Mj>lb);
%     if length(validInds)<=r+1
%         validInds=1:d;
%     end
%     disp(length(validInds));
...
%         info.itr = info.itr + 1;
        ...
        oi = randsample(d,r+1);
%         oi = randsample(validInds,r+1);
        Uoi = Uj(oi,:);
        xoi = Mj(oi);
        Coeffstmp = (Uoi'*Uoi)\Uoi'*xoi;
