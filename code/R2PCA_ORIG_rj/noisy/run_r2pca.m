function [ err, elaptime, mu, s_err, L_rank ] = run_r2pca(d, N, r, p, mu, v, k, sigma_n)
% 
...
fprintf(' -on LoR \t'); ltic = tic;
% Uhat = LoR_noisy_par(M, r, k, sigma_n); 
[Uhat] = LoR_noisy(M, r, k, sigma_n); 
ltoc = toc(ltic);
fprintf(' -done LoR = %.2f sec \n',ltoc);


% 2nd part of R2PCA: recover coefficients
fprintf(' -on Sp \t'); stic=tic;
Coeffs = Sp_noisy_par(M, Uhat, k, sigma_n, r); 
stoc=toc(stic);
fprintf(' -done Sp = %.2f sec \n',stoc);

% % 2nd part of R2PCA: recover coefficients
% fprintf(' -on Sp \t'); stic=tic;
% Coeffsmod = Sp_noisy(M, Uhatmod, k, sigma_n, r); 
% stoc=toc(stic);
% fprintf(' -done Sp = %.2f sec \n',stoc);
...
L_rank = rank(Lhat);
s_err = norm(S-Shat,'fro')/norm(S,'fro');




% % Now recover low rank component
% Lhatmod = Uhatmod * Coeffsmod;        % Recover low-rank compoment
% % And recover sparse component
% Shatmod = M - Lhatmod;
% 
% % Lerr = norm(L-Lhat,'fro')/norm(L,'fro');
% errmod = norm(L-Lhatmod,'fro')/norm(L,'fro');
% 
% Lmod_rank = rank(Lhatmod);
% s_errmod = norm(S-Shatmod,'fro')/norm(S,'fro');
