...
vidPath = [vidDir filesep 'Video_002.m4v'];
vidObj = VideoReader(vidPath);

frames = read(vidObj,[1 200]);
...
% k=2*r;
% tic
% [L, S, U, T, info] = nR2PCA(images, r, k, tol, timelimit, verbose);
% toc
...
% S_(S_>0)=1;
% 
% figure; imshow(reshape(L(150,:),h,w),[0 1]);
% figure; imshow(abs(reshape(S(150,:),h,w)),[0 1]);
