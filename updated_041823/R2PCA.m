function [L,S,Coeffs,Uhat] = R2PCA(M,r)
...
%     iterRankVals(end+1)=(auxrank);
%     if mod(length(iterRankVals),100)==0, figure(1); plot(iterRankVals); drawnow; end
...
%         resp_vals(end+1) = norm(xoiPerp)/norm(xoi);
        ...
        if norm(xoiPerp)/norm(xoi) < tol %1e-9
            resp = 1;
        end
