...
parfor outerind=1:d-r
    if mod(outerind,100)==0, fprintf(' -%d\n',outerind); end
...
atoc = toc(atic);
fprintf(' - Finished LoR_par, elap time = %g s\n',atoc);
...
tic; [Uhat,~,~] = svd(Ahat); toc
Uhat = Uhat(:,d-r+1:d);

% Ahat = sparse(Ahat);
% tic; [Uhat,~,~] = svds((Ahat),r,'smallest'); toc
